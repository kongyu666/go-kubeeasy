...
import (
	"fmt"
	"github.com/windvalley/gossh/internal/scripts"
...
// HostPreEnvCmd represents the script command
var HostPreEnvCmd = &cobra.Command{
	Use:   "hostpre [HOST...]",
...
func init() {
	HostPreEnvCmd.Flags().StringVarP(&scriptFile, "execute", "e", "",
		"a shell script to be executed on target hosts",
	)

	HostPreEnvCmd.Flags().StringVarP(&destPath, "dest-path", "d", "/tmp",
		"path of target hosts where the script will be copied to",
	)

	HostPreEnvCmd.Flags().BoolVarP(&remove, "remove", "r", false,
		"remove the copied script after execution",
	)

	HostPreEnvCmd.Flags().BoolVarP(&force, "force", "F", false,
		"allow overwrite script file if it already exists on target hosts",
	)

	rootCmd.AddCommand(HostPreEnvCmd)
}
